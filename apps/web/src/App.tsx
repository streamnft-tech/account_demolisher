      ...
      <main className="workspace">
        <section className="workspaceHero">
          <div>
            ...
            <p>
              Use the existing health scanner to review blockers, then continue toward a safe merge when the account is
              ready.
            </p>
          </div>
        ...
        <section className="cardGrid">
          <section className="card">
            ...
            <p className="hint">
              Backend scans Horizon for merge blockers. Pick <strong>testnet</strong> or <strong>mainnet</strong>.
            </p>
        ...
        {accountKnown ? (
          <section className="cardGrid">
            <section className="card">
              ...
              {blocking.length === 0 ? (
                <p className="summary ok">
                  No blocking issues from the summary list. Use the checklist above for full coverage, then re-run the
                  scan.
                </p>
              ) : (
                <>
                  <p className="hint">
                    Each item needs signed transactions. Fix externally if needed, then use <strong>Re-run health check</strong>.
                  </p>
            ...
            <section className="card">
              <h2 className="cardTitle">Step 4 - Merge</h2>
              <p className="hint">Enabled only when the scan reports zero blocking issues and the destination is valid.</p>
              <button
                type="button"
                className="btn danger"
                disabled={!canDemolish}
                title={canDemolish ? undefined : "Clear blockers and set destination first"}
              >
                Merge account
              </button>
              {!canDemolish ? (
